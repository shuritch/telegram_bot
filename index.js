...
class person {
    ...
    popular_words = () => {
        ...
        return max_words.reduce((el, index) => { return el + ' ' + index });
    }
    mood = () => {
        let mood = (this.polarity.reduce((a, b) => (a + b)) / this.polarity.length) * (this.subjectivity.reduce((a, b) => (a + b)) / this.subjectivity.length);
    ...
    favorite_phrase = () => {
        var mas = (this.messages).reduce(function(mas, s) {
            mas.freq[s] = (mas.freq[s] || 0) + 1;
            if(!mas.freq[mas.most] || mas.freq[s] > mas.freq[mas.most])
            mas.most = s;
            return mas;
        }, { freq: {}, most: '' });
...
const start = () => {
    ...
    bot.on('message', async msg => {
        ...
        if(text === '/start'){ 
            ...
            return bot.sendMessage(chatId, 'Бот запущен, чтобы воспользоваться анализом переписки - отправьте её JSON формат текстом или файлом.');
        } else if(text === '/info'){ 
            return bot.sendMessage(chatId, 'Бот реализован для проекта "психолингвистический анализ человека по переписке в Telegram".');
        } else if(text === '/how_to_use'){ 
            await bot.sendMessage(chatId, 'Откройте нужный вам диалог, в верхнем правом углу вы увидите 3 точки, нажав на эту кнопку выбирите экспорт истрии чата. В открывшимся окне выбирите формат - JSON, нажмите кнопку экспорт. Скопируйте всё содержимое файла и отправьте в этот диалог.');
            ...
            if(document.mime_type != 'application/json') return bot.sendMessage(chatId, 'Поддерживаются только файлы типа JSON.');
                ...
                const request = https.get(link, (response) => {
                    ...
                    response.on('end', function () {
                        fs.readFile(`file_${file_name}.json`, "utf8", async (error,data) => {
                            ...
                            if(!isJson(data)){ 
                                fs.unlink(`file_${file_name}.json`, (err) => { if (err) console.log(err); });
...
tokenizer = (str) => { return str.replace(/[.,\/#!?$%\^&\*;:{}=\-_`~()]/g,"").replace(/\s+/g, " ").split(' '); };
(function () {
